...
void main() {
  ...
  test('When the cache is empty and getRecipe is called, then null is returned', () {
    var url = Uri.parse("https://example.org/recipe");
    var recipe = RecipeCache().getRecipe(url);
    expect(recipe, isNull);
  });

  test('When the cache contains a recipe and getRecipe is called with the same'
      'properties, then the recipe is returned', () {
    var url = Uri.parse("https://example.org/recipe");
    var recipe = const Recipe(ingredients: [], name: "test name", servings: 2);
  ...
  test('When the cache is empty and addRecipe is called, then recipe is added '
      'to the cache.', () {
    var url = Uri.parse("https://example.org/recipe");
    var recipe = const Recipe(ingredients: [], name: "test name", servings: 2);
  ...
  test('When the cache contains a recipe and addRecipe is called with the same '
    ...
    var newRecipe = const Recipe(
      ingredients: [Ingredient(amount: 12.34, unit: "test unit", name: "test name")],
      name: "test name",
      servings: 2,
    );
  ...
  test('When the redirect cache is empty and addRedirect is called, then redirect '
    ...
    var cachedRedirectUrl = RecipeCache().redirects[RecipeCache().getKey(url)];
    expect(cachedRedirectUrl, isNotNull);
  });

  test('When redirect cache contains redirect and getRedirect is called, then '
