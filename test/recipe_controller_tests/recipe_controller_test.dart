...
void main() {
  ...
  test('When recipe is parsed again, then the cached recipe is used to create the '
      'RecipeParsingResult', () async {
    ...
    var result = await RecipeController()
        .collectRecipes(recipeParsingJobs: [job], language: "de")
    ...
    var secondResult = await RecipeController()
        .collectRecipes(recipeParsingJobs: [job.copyWith(servings: 2)], language: "de")
        .then((value) => value.first);
  ...
  test('When modification and note are stored for recipe, then the modification is'
      ' applied and a log created', () async {
    ...
    await LocalStorageController().setAdditionalRecipeInformation(
      AdditionalRecipeInformation(
        ...
        recipeModification: const RecipeModification(
          servings: 4,
          modifiedIngredients: [Ingredient(amount: 4, unit: "", name: "Zucchini")],
        ),
      ),
    );
    ...
    var modifiedResult = await RecipeController().applyRecipeModification(result, recipeUrlOrigin);

    expect(modifiedResult.recipe, isNotNull);
    var recipe = modifiedResult.recipe!;
    var ingredient = recipe.ingredients.where((element) => element.name == "Zucchini").first;
  ...
  test('When note is stored for recipe, then a log is created', () async {
    var recipeUrlOrigin = "test origin";
  ...
  test('When recipe is parsed and only modification is stored, then no log is '
      'created', () async {
    var url = Uri.parse("http://mobile.kptncook.com/recipe/pinterest/50d87d41");

    var job = RecipeController().createRecipeParsingJob(url: url, servings: 4, language: "de");
    ...
    expect(result.logs.whereType<AdditionalRecipeInformationJobLog>(), isEmpty);
  });
}
