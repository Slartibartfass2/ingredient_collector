...
import 'package:ingredient_collector/src/recipe_parser/recipe_parser.dart' show ChefkochParser;
...
void main() {
  ...
  test(
    ...
    () async => testParsingTestFiles("./test/recipe_parser_tests/parser_test_files/chefkoch"),
  ...
  group('Recipe parsing', () {
    ...
    test('When the servings element is missing, then parsing returns errors', () {
      ...
      var document4 = Document.html("""
        ...
        $ingredientsElement
        """);
      expectRecipeParsingErrors(parser, [document1, document2, document3, document4]);
    });
  ...
  group('Ingredient parsing', () {
    ...
    test('When element with amount and unit is parsed, then parsing is '
      ...
      expect(
        ...
        equals(const Ingredient(amount: 200, unit: "ml", name: "Weißwein, trockener")),
      );
    });
    ...
    test('When element with only amount is parsed, then parsing is successful', () {
      ...
      expect(
        result.ingredients.first,
        equals(const Ingredient(amount: 0.5, unit: "", name: "Zitrone(n), Saft davon")),
      );
    });
  });
}
